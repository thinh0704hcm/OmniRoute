 ...
 */
function estimateBodyBytesWithoutToolSchemas(body: unknown, byteLimit = ESTIMATE_SIZE_BYTE_LIMIT): number {
  const full = estimateSizeFast(body, byteLimit);
...
export function extractAdmissionCostFeatures(
  ...
  const bodyBytes = estimateBodyBytesWithoutToolSchemas(
    body,
    cost.bodyBytesPerUnit * cost.maxRequestCost
  );
  const layers = featureLayers(body);
