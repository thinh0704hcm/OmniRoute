...
const DEFAULT_MODELS = ["pool-haiku", "pool-sonnet", "pool-opus", "pool-fable"];
...
async function probeModels(baseUrl, expectedModels) {
  try {
    ...
    console.log(
      `[probe] models unauth=${unauthenticated.status} auth=${authenticated.status} allPresent=${allPresent} count=${ids?.length}`
    );
    if (!allPresent && Array.isArray(expectedModels)) {
      console.log(
        `[probe] expected models: ${JSON.stringify(expectedModels)}, missing: ${JSON.stringify(expectedModels.filter((m) => !ids?.includes(m)))}`
      );
    }
    return {
    ...
    console.log(`[probe] models error: ${err?.message || err}`);
    return {
...
async function runTrafficProbes(baseUrl, models) {
  ...
  const comboModel = models.includes("pool-sonnet") ? "pool-sonnet" : models[0];
