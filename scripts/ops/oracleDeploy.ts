...
/**
 * Legacy flattened gate shape retained for callers which only run the old
 * application probes. New production adapters must return LocalRuntimeGate
 * and PublicGate separately; evaluateRuntimeGate dispatches to the strict
 * contract when one of its explicit fields is present.
 */
export interface RuntimeGate {
  ...
  // Explicit-contract fields are optional only for source compatibility with
  // old unit fixtures. They are required at runtime by evaluateLocalRuntimeGate.
  containerName?: string | null;
  containerStatus?: string | null;
  healthStatus?: string | null;
  dashboardOk?: boolean | null;
...
function explicitLocalContract(value: unknown): value is Partial<LocalRuntimeGate> {
  if (!value || typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
  return (
    "containerName" in record ||
    "containerStatus" in record ||
    "healthStatus" in record ||
    "dashboardOk" in record ||
    "healthOk" in record ||
    "apiModelsOk" in record ||
    "liveWsOk" in record
  );
}
...
/**
 * Backward-compatible evaluator for the original flattened gate. New local
 * contracts are strict and dispatched automatically when their explicit fields
 * are present.
 */
export function evaluateRuntimeGate(input: RuntimeGate | LocalRuntimeGate): RuntimeGateVerdict {
  if (explicitLocalContract(input)) return evaluateLocalRuntimeGate(input);
  const gate = input as RuntimeGate;
  const failures: string[] = [];
  if (gate.healthy !== true) failures.push("health endpoint is not healthy");
  if (!strictNonEmptyString(gate.actualBuildSha)) {
    failures.push("image build SHA is missing");
  } else if (gate.actualBuildSha !== gate.expectedBuildSha) {
    failures.push("image build SHA does not match the candidate");
  }
  if (gate.actualImageId !== gate.expectedImageId) {
    failures.push("running image ID does not match the candidate");
  }
  if (gate.restartCount !== 0) failures.push("container restarted during qualification");
  if (gate.oomKilled !== false) failures.push("container was OOMKilled");
  if (gate.memoryBytes !== EXPECTED_MEMORY_BYTES) {
    failures.push(`memory limit is ${gate.memoryBytes} bytes, expected ${EXPECTED_MEMORY_BYTES}`);
  }
  if (gate.nanoCpus !== EXPECTED_NANO_CPUS) {
    failures.push(`CPU limit is ${gate.nanoCpus}, expected ${EXPECTED_NANO_CPUS}`);
  }
  if (gate.completionOk !== true) failures.push("completion probe failed");
  if (gate.streamingOk !== true) failures.push("streaming probe failed");
  if (gate.mixedCaseToolOk !== true) failures.push("mixed-case tool continuation failed");
  if (gate.comboOk !== true) failures.push("combo-routing probe failed");
  if (gate.callLogAdvanced !== true) failures.push("call_logs did not advance");
  return { ok: failures.length === 0, failures };
}
