...
| `aa-benchmark` 📊   | Rank targets by [Artificial Analysis](https://artificialanalysis.ai/) benchmark scores (best first); `config.aaMetric` picks intelligence / coding / math / speed |

⭐ = New in v3.8.0 · 🧬 = New in v3.8.36 · 📊 = New in v3.8.50

> `aa-benchmark` needs `ARTIFICIAL_ANALYSIS_API_KEY` set so the daily sync
> (`src/lib/artificialAnalysisSync.ts`) can populate scores; targets without a
> synced score keep their original order at the end of the chain. Benchmark
