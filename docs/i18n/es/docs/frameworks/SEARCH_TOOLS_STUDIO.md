...
> **Funcionalidad:** Search Tools Studio — espacio de trabajo unificado de herramientas web para `/dashboard/search-tools`.
> **Planes:** `18-search-tools-studio-redesign.plan.md` + `_orchestration/master-plan-group-C.md`
...
Search Tools Studio transforma `/dashboard/search-tools`, que pasa de ser un entorno básico de pruebas de búsqueda a un
...
`ProviderCatalog.tsx` expone la lista completa de proveedores de `GET /api/search/providers`
...
El estado se **determina en el momento de la solicitud** comprobando si existen credenciales y si
todas las claves se encuentran actualmente en período de espera.

---

## Exportación de código

`ExportCodeModal` (importado desde Playground Studio) + `codeExport.ts` generan
