...
> **功能：** Playground Studio — 適用於 `/dashboard/playground` 的統一 AI 測試工作區。
> **計畫：** `17-playground-studio-redesign.plan.md` + `_orchestration/master-plan-group-C.md`
> **狀態：** 已於 v3.8.6 發布
...
- `ToolsBuilder.tsx` — 新增／編輯／移除 `tools[]`，且每個工具皆有 JSON Schema 編輯器。
  透過 `ToolDefinitionSchema`（Zod）驗證參數。
- `StructuredOutputEditor.tsx` — 切換 JSON 模式 + JSON Schema 編輯器。
  透過 `StructuredOutputSchema`（Zod）依據結構描述驗證回應。
...
`ExportCodeModal.tsx` 使用 `codeExport.ts`，根據目前的 `PlaygroundState` 產生 curl / Python / TypeScript 程式碼片段。
