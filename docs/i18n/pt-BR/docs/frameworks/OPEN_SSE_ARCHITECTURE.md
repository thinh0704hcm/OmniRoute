...
> **Resumindo**: `open-sse/` é o mecanismo central de streaming que processa todas as solicitações a LLMs no OmniRoute. Ele contém cerca de 1.400 arquivos que implementam o pipeline de solicitações, executores, serviços, servidor MCP e camada de tradução. Este guia explica como essas partes se encaixam.

**Fonte:** `open-sse/` (pacote do workspace, cerca de 1.440 arquivos `.ts` que não são de teste)
...
### Etapa 1: Roteamento (services/combo.ts)
...
❌ **Chamadas síncronas ao banco de dados em `combo.ts`** — pré-calcule e armazene em cache
...
- [REPOSITORY_MAP.md](../architecture/REPOSITORY_MAP.md) — detalhamento diretório por diretório
- [AUTO-COMBO.md](../routing/AUTO-COMBO.md) — pontuação de 16 fatores
- [MCP-SERVER.md](./MCP-SERVER.md) — servidor MCP
- [A2A-SERVER.md](./A2A-SERVER.md) — servidor A2A
- Código-fonte: `open-sse/` (mais de 400 arquivos, ~143 mil linhas de código)
