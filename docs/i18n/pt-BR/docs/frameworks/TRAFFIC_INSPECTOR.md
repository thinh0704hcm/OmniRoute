...
Traffic Inspector é o depurador de tráfego HTTPS integrado do OmniRoute — uma ferramenta semelhante ao Charles Proxy / mitmweb / HTTP Toolkit que é **compatível com LLMs** e **compatível com agentes**. Ele está disponível em `/dashboard/tools/traffic-inspector` e recebe tráfego em tempo real de até 5 fontes de captura simultâneas.

**Localização no dashboard:** `/dashboard/tools/traffic-inspector`
**Grupo da barra lateral:** Ferramentas (após AgentBridge)
**Veja também:** [`AGENTBRIDGE.md`](./AGENTBRIDGE.md) — AgentBridge é o modo de captura 1.
...
**Fonte:** Manipuladores do AgentBridge (`src/mitm/handlers/base.ts`)
...
**Fonte:** Lista de hosts definida pelo usuário (tabela `inspector_custom_hosts`)
...
**Fonte:** Aplicações que usam as variáveis de ambiente `HTTP_PROXY`/`HTTPS_PROXY`
...
**Fonte:** Configurações de proxy do sistema operacional (aplicam-se a todas as aplicações da máquina)
**Mecanismo:** Usa APIs do sistema operacional para redirecionar todo o tráfego HTTP/HTTPS pelo listener HTTP_PROXY:
...
- **Windows:** `netsh winhttp set proxy 127.0.0.1:8080`
  **Alcance:** Todas as aplicações da máquina que respeitem as configurações de proxy do sistema
  **Observação:** `source` = `"system-proxy"`
...
**Fonte:** TPROXY do kernel + roteamento baseado em políticas (`src/mitm/tproxy/`)
...
### 3.2 Lista de requisições (painel esquerdo)
...
enquanto a reconstrução segue os esquemas públicos de streaming da [OpenAI](https://platform.openai.com/docs/api-reference/chat/create),
...
locais de caixa-preta e pelos esquemas públicos de mensagens da OpenAI, Anthropic e Gemini; nenhuma
implementação de código-fonte upstream é usada.
...
O campo `status` da solicitação é `number | "in-flight" | "error"` — uma entrada é
...
O estado do controle fica em `useTrafficFilters` (os hooks do painel do inspetor) e
é combinado com os outros filtros (perfil, host, agente, origem, status, contexto).

### 4.7 Atribuição de processo (Linux)

No Linux, cada solicitação interceptada pode ser atribuída ao **processo local de
...
`src/mitm/inspector/processAttribution.ts` mapeia a porta efêmera do _cliente_
...
1. Lê `/proc/net/tcp` e `/proc/net/tcp6` para encontrar o inode do socket correspondente à
   porta (`parseProcNetTcpForInode`, um analisador puro testável com fixtures).
...
> A **descriptografia TPROXY** (modo de captura 5) é controlada por uma rota
