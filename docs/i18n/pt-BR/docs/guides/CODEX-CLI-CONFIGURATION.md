...
### macOS: Codex incluído no aplicativo ChatGPT

Se você instalou o Codex por meio do aplicativo ChatGPT para desktop, o binário
...
O Codex verifica se a variável de ambiente indicada por `env_key` existe
...
Os perfis permitem alternar o modelo + janela de contexto com uma única flag. Cada perfil é um arquivo simples
`~/.codex/<name>.config.toml` sobreposto ao `config.toml` base.

> **Regra de nomenclatura (Codex CLI v0.137+):** o arquivo deve ser `~/.codex/<name>.config.toml` — **sem o prefixo `profile-`**.
> O CLI resolve `-p kimi-k27` → `~/.codex/kimi-k27.config.toml`. Se o arquivo não for encontrado, a configuração padrão será aplicada silenciosamente.
...
O Codex também é um destino dos dois pontos de entrada genéricos orientados por manifesto
...
## Novos recursos da CLI do Codex (v0.138–v0.141)
...
| --------------------------------- | ------------------------------------- | ---------------------------------- |
| `tool_output_token_limit = 32768` | _(não exposto diretamente)_           | Limite de histórico por ferramenta |
...
## Referência rápida — flags da CLI
...
Discussões relacionadas: [#7126](https://github.com/diegosouzapw/OmniRoute/discussions/7126) (interrupções em tarefas longas), [#5718](https://github.com/diegosouzapw/OmniRoute/discussions/5718) (por que a afinidade vem desabilitada por padrão). Acompanhamento: [#7287](https://github.com/diegosouzapw/OmniRoute/issues/7287).
...
**Padrão:** `STREAM_IDLE_TIMEOUT_MS = 600000` (10 minutos). Quando não definido, ele é herdado de `REQUEST_TIMEOUT_MS`; o valor-base compartilhado também é 600000. Consulte [`docs/guides/SETUP_GUIDE.md`](SETUP_GUIDE.md) → Tempos limite.
...
3. Mantenha o `config.toml` habitual do Codex (`wire_api = "responses"`, `base_url` correto, `OMNIROUTE_API_KEY`) — não existem opções de afinidade/inatividade no lado do Codex para esses dois comportamentos.
4. Reinicie o OmniRoute e, em seguida, inicie a tarefa longa do Codex.
...
Consulte [Tarefas de longa duração](#long-running-tasks). Habilite a afinidade de sessão (com TTL maior que a duração da tarefa) e aumente ou desabilite `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Pesquise `Idle timeout: no data from` nos logs do OmniRoute.
