...
Scorecard Branch-Protection 점수를 0에서 올립니다. `enforce_admins:false`로 설정하면 기존
포워드 병합 흐름을 계속 사용할 수 있으며, 안정화되면 `true`로 강화하세요.
