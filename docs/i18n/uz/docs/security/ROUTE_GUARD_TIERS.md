...
1. Yoʻl prefiksini `src/server/authz/routeGuard.ts` faylidagi `LOCAL_ONLY_API_PREFIXES` roʻyxatiga qoʻshing
2. `tests/unit/authz/routeGuard.test.ts` fayliga yangi prefiks uchun `isLocalOnlyPath()` true qaytarishini tasdiqlovchi test qoʻshing
3. **Bu bosqichni hech qachon oʻtkazib yubormang** — `CLAUDE.md` faylidagi 15-qatʼiy qoidaga qarang
4. Qaror qiling: bu marshrut `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` roʻyxatiga HAM kirishi kerakmi?
   Standart javob — **yoʻq**. Faqat marshrutni manage-scope egasiga ochish xavfsiz boʻlgandagina (yaʼni foydalanuvchi boshqaradigan ixtiyoriy kodni ishga tushirmasa) uni ixtiyoriy ravishda kiriting.

## Manage-scope orqali chetlab oʻtish mumkin boʻlgan yoʻl qoʻshish

1. Marshrut foydalanuvchi taqdim etgan kod yoki buyruqlarni bajarmasligini tasdiqlang. Agar bajarsa, toʻxtang — bu istisno mexanizmi mos vosita emas.
2. Prefiksni `src/server/authz/routeGuard.ts` faylidagi `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` roʻyxatiga qoʻshing
3. `tests/unit/authz/management-policy.test.ts` faylida barcha toʻrtta soʻrov shakli uchun qamrov qoʻshing: Bearer yoʻq (403), manage Bearer (ruxsat berish), manage boʻlmagan Bearer (403) hamda manage Bearer bilan ham `/api/cli-tools/runtime/*` qatʼiy loopback boʻlib qolishini tekshiruvchi har bir prefiksga oid regressiya testi.

## Fayllar

| Fayl                                         | Maqsadi                                 |
| -------------------------------------------- | --------------------------------------- |
| `src/server/authz/routeGuard.ts`             | Konstantalar va yordamchi funksiyalar   |
| `src/server/authz/policies/management.ts`    | Baholash mantigʻi                       |
| `tests/unit/authz/routeGuard.test.ts`        | Daraja yordamchilari uchun unit-testlar |
| `tests/unit/authz/management-policy.test.ts` | evaluate() uchun unit-testlar           |

## OpenAPIʼda xavfsizlik darajalarini hujjatlashtirish

`docs/openapi.yaml` fayliga yangi marshrut qoʻshayotganda, agar marshrut `routeGuard.ts` tomonidan tasniflangan boʻlsa, unga mos vendor kengaytmasini qoʻllang:

| routeGuard.ts tasnifi                           | YAML annotatsiyasi         | Majburiy qoʻllash                                                        |
| ----------------------------------------------- | -------------------------- | ------------------------------------------------------------------------ |
| `LOCAL_ONLY_API_PREFIXES`                       | `x-loopback-only: true`    | Loopback boʻlmagan ulanishlardan soʻzsiz bloklanadi                      |
| `ALWAYS_PROTECTED_API_PATHS`                    | `x-always-protected: true` | Hatto `requireLogin=false` boʻlganda ham autentifikatsiya talab qilinadi |
| Ichki admin/debug marshruti                     | `x-internal: true`         | Standart holatda /dashboard/api-endpoints sahifasidan yashiriladi        |
| Hech biri (ommaviy / standart autentifikatsiya) | (annotatsiya kerak emas)   | Standart `requireLogin` orqali boshqariladigan kirish                    |

### Tekshirish

Ikki skript YAML annotatsiyalari va `routeGuard.ts` oʻrtasidagi muvofiqlikni taʼminlaydi:

- `scripts/check/check-openapi-coverage.mjs` — qamrov 99% dan past boʻlsa, xato bilan yakunlanadi
- `scripts/check/check-openapi-security-tiers.mjs` — `x-loopback-only` yoki
  `x-always-protected` annotatsiyalari kompilyatsiya vaqtidagi konstantalardan farq qilsa, xato bilan yakunlanadi

Har ikkala skript ham pre-commit hookʼida va CIʼda ishga tushadi.

### Notoʻgʻri ijobiy natija qoidasi

Agar `routeGuard.ts` konstantasida MAVJUD BOʻLMAGAN marshrutga `x-always-protected` yoki `x-loopback-only` annotatsiyasi qoʻyilgan boʻlsa, qamrov skripti xato bilan yakunlanadi. Toʻgʻri yechim har doim YAMLʼni `routeGuard.ts` amalda taʼminlaydigan qoidalarga moslashtirishdir — majburiy qoʻllash mantigʻini ham amalga oshirmasdan marshrutlarni `routeGuard.ts` fayliga qoʻshish emas.

---

## Shuningdek qarang

- `docs/security/CLI_TOKEN.md` — CLI qurilma identifikatori tokeni
- `docs/architecture/AUTHZ_GUIDE.md` — toʻliq avtorizatsiya konveyeri
- `docs/frameworks/MCP-SERVER.md` — MCP server transportlari va qamrov doiralari
