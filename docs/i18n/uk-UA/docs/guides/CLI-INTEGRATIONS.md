...
для програмування (Codex, Claude Code, OpenCode, Cline, …) на використання OmniRoute як бекенду — тож
інструмент звертається до **однієї** кінцевої точки, а OmniRoute спрямовує запити до потрібного провайдера з
автоматичним резервним перемиканням. Кожна команда зчитує **актуальний** каталог моделей із запущеного
екземпляра OmniRoute (локального або віддаленого) і записує власний конфігураційний файл інструмента на **вашому**
комп’ютері. Там, де інструмент це підтримує, посилання на ключ API задається через змінну середовища. Команди, які зберігають локальний для інструмента файл середовища, зазначено нижче.

Також доступний універсальний засіб запуску — `omniroute run <target>` — який запускає
`claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` або `gemini` з
підставленими належними змінними середовища, взагалі не записуючи жодної конфігурації. Цілі та їхні
псевдоніми беруться з канонічного маніфесту `bin/cli/cli-manifest.mjs`
(`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`,
`open-code`, `qwen-code`, `gemini-cli`), а `omniroute completion` пропонує ті самі
назви цілей, отримані з маніфесту. Застарілі засоби запуску для окремих інструментів —
`omniroute launch` (Claude Code) і `omniroute launch-codex` (Codex) — залишаються
доступними.

Підключення провайдерів доступне з того самого локального або віддаленого контексту. Наведені
нижче команди, орієнтовані насамперед на API, відокремлюють автентифікацію керування від облікових даних провайдера
й ніколи не виводять облікові дані у структурованому форматі:
...
Для сценаріїв віддавайте перевагу `--credential-stdin` або `--credential-env`; параметр `--credential`
збережено для контрольованого локального використання. Команда `providers remove` вимагає `--yes` у
неінтерактивному терміналі, а всі п’ять команд враховують активний контекст або
глобальні параметри `--base-url`/`--api-key`.

Інструкції з одноразового базового ручного налаштування двох найфункціональніших інтеграцій дивіться в
детальних посібниках для кожного інструмента:
...
- [VS Code Copilot Chat](./VSCODE-COPILOT.md) — розширення OmniCopilot; воно також може запускати ці
  команди `setup-*` для вас безпосередньо з редактора
