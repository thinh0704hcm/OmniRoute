...
> **Kurz gesagt**: `open-sse/` ist die zentrale Streaming-Engine, die jede LLM-Anfrage in OmniRoute verarbeitet. Sie enthält etwa 1.400 Dateien, die die Anfrage-Pipeline, Executors, Services, den MCP-Server und die Übersetzungsschicht implementieren. Dieser Leitfaden erklärt, wie die einzelnen Komponenten zusammenwirken.

**Quelle:** `open-sse/` (Workspace-Paket, etwa 1.440 `.ts`-Dateien ohne Tests)
...
### Stufe 1: Routing (services/combo.ts)
...
❌ **Synchrone DB-Aufrufe in `combo.ts`** — vorab berechnen und zwischenspeichern
...
- [ARCHITECTURE.md](../architecture/ARCHITECTURE.md) — übergeordnete Architektur
- [CODEBASE_DOCUMENTATION.md](../architecture/CODEBASE_DOCUMENTATION.md) — technische Referenz
- [REPOSITORY_MAP.md](../architecture/REPOSITORY_MAP.md) — Verzeichnisübersicht
- [AUTO-COMBO.md](../routing/AUTO-COMBO.md) — Bewertung anhand von 16 Faktoren
- [MCP-SERVER.md](./MCP-SERVER.md) — MCP-Server
- [A2A-SERVER.md](./A2A-SERVER.md) — A2A-Server
- Quellcode: `open-sse/` (über 400 Dateien, ca. 143.000 Codezeilen)
