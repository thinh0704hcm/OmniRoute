...
Traffic Inspector ist der integrierte HTTPS-Traffic-Debugger von OmniRoute — ein mit Charles Proxy / mitmweb / HTTP Toolkit vergleichbares Tool, das **LLM-fähig** und **Agenten-fähig** ist. Es befindet sich unter `/dashboard/tools/traffic-inspector` und empfängt Live-Traffic von bis zu 5 gleichzeitigen Erfassungsquellen.

**Dashboard-Speicherort:** `/dashboard/tools/traffic-inspector`
**Seitenleistengruppe:** Tools (nach AgentBridge)
**Siehe auch:** [`AGENTBRIDGE.md`](./AGENTBRIDGE.md) — AgentBridge ist Erfassungsmodus 1.

---

## §1 Überblick

### Was Traffic Inspector einzigartig macht
...
**Quelle:** AgentBridge-Handler (`src/mitm/handlers/base.ts`)
...
**Quelle:** Benutzerdefinierte Hostliste (Tabelle `inspector_custom_hosts`)
...
**Quelle:** Anwendungen, die die Umgebungsvariablen `HTTP_PROXY`/`HTTPS_PROXY` verwenden
...
**Quelle:** Proxy-Einstellungen des Betriebssystems (gelten für alle Anwendungen auf dem Rechner)
**Mechanismus:** Verwendet Betriebssystem-APIs, um den gesamten HTTP/HTTPS-Datenverkehr durch den HTTP_PROXY-Listener umzuleiten:
...
- **Windows:** `netsh winhttp set proxy 127.0.0.1:8080`
  **Reichweite:** Jede Anwendung auf dem Rechner, die die systemweiten Proxy-Einstellungen berücksichtigt
  **Hinweis:** `source` = `"system-proxy"`
...
**Quelle:** Kernel-TPROXY + richtlinienbasiertes Routing (`src/mitm/tproxy/`)
...
### 3.2 Anfrageliste (linker Bereich)
...
**Unabhängige Clean-Room-Implementierung.** Das Parsen von Ereignissen folgt dem
[WHATWG-Algorithmus für Server-Sent Events](https://html.spec.whatwg.org/multipage/server-sent-events.html#parsing-an-event-stream),
während die Rekonstruktion den öffentlichen Streaming-Schemata von
[OpenAI](https://platform.openai.com/docs/api-reference/chat/create),
[Anthropic](https://platform.claude.com/docs/en/build-with-claude/streaming) und
...
- **OpenAI**: akkumuliert Auswahlmöglichkeiten/Tool-Aufrufe von Chat Completions sowie Ausgabeelemente der Responses API
  nach Index
...
**Unabhängige Clean-Room-Implementierung.** Die Normalisierung wird durch lokale Black-Box-
Verträge und die öffentlichen Nachrichtenschemata von OpenAI, Anthropic und Gemini definiert; es wird kein
Quellcode einer vorgelagerten Implementierung verwendet.
...
### 4.6 Live-Filter für laufende Anfragen

Das Anfragefeld `status` hat den Typ `number | "in-flight" | "error"` — ein Eintrag wird
...
Der Zustand des Umschalters befindet sich in `useTrafficFilters` (den Hooks des Inspector-Dashboards) und
wird mit den anderen Filtern kombiniert (Profil, Host, Agent, Quelle, Status, Kontext).

### 4.7 Prozesszuordnung (Linux)

Unter Linux kann jede abgefangene Anfrage dem **ursprünglichen lokalen
...
`src/mitm/inspector/processAttribution.ts` ordnet den ephemeren _Client_-Port
der Verbindung einer PID und einem Namen zu, indem:

1. `/proc/net/tcp` und `/proc/net/tcp6` gelesen werden, um den Socket-Inode für den
   Port zu finden (`parseProcNetTcpForInode`, ein reiner, mit Fixtures testbarer Parser).
...
### System-Proxy wurde nicht zurückgesetzt
...
> **TPROXY-Entschlüsselung** (Erfassungsmodus 5) wird über eine **separate** Route unter dem
> AgentBridge-Präfix gesteuert — `GET / POST / DELETE /api/tools/agent-bridge/tproxy` — und nicht
> unter `/api/tools/traffic-inspector/`. Siehe
> `docs/security/MITM-TPROXY-DECRYPT.md` (git; nicht in `/docs` kompiliert).
