...
Testen Sie außerdem jede betroffene Anfragefamilie: Chat, Responses, Bilder, Embeddings, Audio oder Video.
Prüfen Sie generierte Katalog- und Golden-Diffs als Vertragsänderungen; übernehmen Sie sie nicht ungeprüft.
...
npm run build:cli             # generierte/gebündelte CLI-Änderungen
npm run check:pack-policy     # Änderungen an der Paketoberfläche
npm run lint
```
