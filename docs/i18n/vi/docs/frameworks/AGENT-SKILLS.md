# OmniRoute Agent Skills Catalog (Tiếng Việt)
...
skills/{id}/SKILL.md                   — Các tệp markdown được tạo + tuyển chọn (tổng cộng 45 tệp)
...
src/app/api/agent-skills/
  ...
  generate/route.ts                    — POST /api/agent-skills/generate (yêu cầu xác thực)

open-sse/mcp-server/tools/agentSkillTools.ts  — 3 công cụ MCP (liệt kê, lấy, độ bao phủ)
...
## Phần tùy chỉnh (được giữ nguyên qua các lần tạo lại)
...
Trình tạo giữ nguyên nội dung nằm giữa `<!-- skill:custom-start -->` và `<!-- skill:custom-end -->` khi tạo lại. Mười skill có các khối tùy chỉnh được tuyển chọn:
...
Kỹ năng A2A `list-capabilities` trả về danh mục đầy đủ gồm 45 kỹ năng dưới dạng một tạo tác bảng markdown. Các trình điều phối bên ngoài có thể gọi kỹ năng này qua:
...
// result.skills → mảng AgentSkill, mỗi phần tử có rawUrl
```
...
Phản hồi của trình tạo là một `GeneratorReport`:
