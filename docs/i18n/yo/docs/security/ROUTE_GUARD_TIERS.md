...
managementPolicy.evaluate(ctx)
  1. isLocalOnlyPath(path)?
     → loopback                                  → tẹ̀síwájú
     → non-loopback, Bearer manage-scope
        ÀTI isLocalOnlyBypassableByManageScope   → gba láàyè (management_key)
     → bí bẹ́ẹ̀ kọ́                              → kọ̀ 403 LOCAL_ONLY
  2. isInternalModelSyncRequest(ctx)?
     → gba láàyè (system)
  3. hasValidCliToken(headers)?
     → gba láàyè (cli) [loopback + àyẹ̀wò HMAC timingSafeEqual]
  4. isAlwaysProtectedPath(path) tàbí requireLogin=true?
     → isDashboardSessionAuthenticated?
        → gba láàyè (dashboard_session)
     → Bearer manage-scope lórí ọ̀nà tí a kò lè yà sílẹ̀?
        → gba láàyè (management_key)
     → kọ̀ 401/403
  5. requireLogin=false?
     → gba láàyè (anonymous)
```

Ẹ̀ka manage-scope ní Ìgbésẹ̀ 1 nìkan ni ọ̀nà ìfàṣẹsí tó lè tẹ́ ìbéèrè
LOCAL_ONLY lọ́rùn; ipò ìkùnà auth-backend máa dá 503 padà (kì í ṣe 403), kí
DB tí ó ti parí má bàa dín kù ní ìdákẹ́jẹ sí “kọ̀”.

## Fífi ọ̀nà tuntun tó lè ṣe spawn kún un

1. Fi ìbẹ̀rẹ̀ ọ̀nà náà kún `LOCAL_ONLY_API_PREFIXES` nínú
   `src/server/authz/routeGuard.ts`
2. Fi ìdánwò kún `tests/unit/authz/routeGuard.test.ts` láti fi ìdí rẹ̀ múlẹ̀ pé
   `isLocalOnlyPath()` máa dá true padà fún ìbẹ̀rẹ̀ tuntun náà
3. **Má ṣe fò ìgbésẹ̀ yìí láé** — wo Òfin Líle #15 nínú `CLAUDE.md`
4. Pinnu: ṣé ó tún yẹ kí ọ̀nà yìí wà nínú `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES`?
   Ìdáhùn àìyípadà ni **rárá**. Yan láti fi sí i nìkan nígbà tí ọ̀nà náà bá ní
   ààbò láti ṣí sí olùdímú manage-scope (ìyẹn ni pé KÒ ṣe spawn kóòdù tí
   olùlò ń darí lọ́nàkọnà).

## Fífi ọ̀nà tí manage-scope lè yà sílẹ̀ kún un

1. Jẹ́rìí pé ọ̀nà náà kò ṣiṣẹ́ kóòdù tàbí àṣẹ tí olùlò pèsè. Bí ó bá
   ṣe bẹ́ẹ̀, dúró — carve-out yìí kì í ṣe irinṣẹ́ tó tọ́.
2. Fi ìbẹ̀rẹ̀ náà kún `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` nínú
   `src/server/authz/routeGuard.ts`
3. Fi ìdánwò àgbáyé kún `tests/unit/authz/management-policy.test.ts` fún gbogbo
   ìrísí ìbéèrè mẹ́rin: kò sí Bearer (403), Bearer manage (gba láàyè), Bearer
   tí kì í ṣe manage (403), àti ìdánwò ìpadàsẹ́yìn fún ìbẹ̀rẹ̀ kọ̀ọ̀kan pé
   `/api/cli-tools/runtime/*` ṣì jẹ́ strict-loopback àní pẹ̀lú Bearer manage.

## Àwọn fáìlì

| Fáìlì                                        | Ète                                       |
| -------------------------------------------- | ----------------------------------------- |
| `src/server/authz/routeGuard.ts`             | Àwọn constant àti iṣẹ́ olùrànlọ́wọ́          |
| `src/server/authz/policies/management.ts`    | Ìlànà ìṣàyẹ̀wò                             |
| `tests/unit/authz/routeGuard.test.ts`        | Àwọn ìdánwò unit fún àwọn olùrànlọ́wọ́ tier |
| `tests/unit/authz/management-policy.test.ts` | Àwọn ìdánwò unit fún evaluate()           |

## Ṣíṣe Àkọsílẹ̀ Àwọn Ìpele Ààbò nínú OpenAPI

Nígbà fífi ọ̀nà tuntun kún `docs/openapi.yaml`, lo vendor extension tó bá a mu
bí `routeGuard.ts` bá pín ọ̀nà náà sí ẹ̀ka kan:

| Ìpínsí routeGuard.ts           | Àlàyé YAML                 | Ìmúṣẹ                                                    |
| ------------------------------ | -------------------------- | -------------------------------------------------------- |
| `LOCAL_ONLY_API_PREFIXES`      | `x-loopback-only: true`    | A dí i lọ́wọ́ non-loopback láìsí àdéhùn                    |
| `ALWAYS_PROTECTED_API_PATHS`   | `x-always-protected: true` | Auth jẹ́ dandan àní pẹ̀lú `requireLogin=false`             |
| Ọ̀nà admin/debug inú            | `x-internal: true`         | A fi pamọ́ kúrò ní /dashboard/api-endpoints lọ́nà àìyípadà |
| Kò sí (gbangba / auth àfọwọ́ṣe) | (kò nílò àlàyé)            | Ìráyè àfọwọ́ṣe tí `requireLogin` ń darí                   |

### Ìfọwọ́sowọ́pọ̀

Script méjì ló ń mú àìyẹsẹ̀ wà láàárín àwọn àlàyé YAML àti `routeGuard.ts`:

- `scripts/check/check-openapi-coverage.mjs` — máa kùnà bí coverage bá kéré ju 99%
- `scripts/check/check-openapi-security-tiers.mjs` — máa kùnà bí àwọn àlàyé
  `x-loopback-only` tàbí `x-always-protected` bá yàtọ̀ sí àwọn constant ní compile-time

Àwọn script méjèèjì máa ń ṣiṣẹ́ nínú pre-commit hook àti nínú CI.

### Òfin False Positive

Bí a bá fi `x-always-protected` tàbí `x-loopback-only` ṣe àlàyé lórí ọ̀nà tí KÒ sí
nínú constant `routeGuard.ts`, script coverage náà máa kùnà. Àtúnṣe tó tọ́ ní gbogbo
ìgbà ni láti mú YAML bá ohun tí `routeGuard.ts` ń fipá mú gangan mu — kì í ṣe láti fi
àwọn ọ̀nà kún `routeGuard.ts` láì tún ṣe àgbékalẹ̀ ìlànà ìmúṣẹ náà.

---

## Tún wo

- `docs/security/CLI_TOKEN.md` — àmì ìdánimọ̀ ẹ̀rọ CLI
- `docs/architecture/AUTHZ_GUIDE.md` — gbogbo ìṣàn iṣẹ́ ìfúniláṣẹ
- `docs/frameworks/MCP-SERVER.md` — àwọn ọ̀nà ìgbékalẹ̀ àti ààlà iṣẹ́ olupin MCP
