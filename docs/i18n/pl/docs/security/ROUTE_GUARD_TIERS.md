...
managementPolicy.evaluate(ctx)
  1. isLocalOnlyPath(path)?
     → loopback                                  → przejdź dalej
     → spoza loopback, Bearer z zakresem manage
        AND isLocalOnlyBypassableByManageScope   → zezwól (management_key)
     → w przeciwnym razie                        → odrzuć 403 LOCAL_ONLY
  2. isInternalModelSyncRequest(ctx)?
     → zezwól (system)
  3. hasValidCliToken(headers)?
     → zezwól (cli) [loopback + sprawdzenie HMAC za pomocą timingSafeEqual]
  4. isAlwaysProtectedPath(path) lub requireLogin=true?
     → isDashboardSessionAuthenticated?
        → zezwól (dashboard_session)
     → Bearer z zakresem manage dla ścieżki, której nie można pominąć?
        → zezwól (management_key)
     → odrzuć 401/403
  5. requireLogin=false?
     → zezwól (anonymous)
```

Gałąź z zakresem manage w kroku 1 jest jedyną uwierzytelnioną ścieżką, która może spełnić wymagania trasy
LOCAL_ONLY; tryb awarii backendu uwierzytelniania zwraca 503 (a nie 403), dzięki czemu
wygasła baza danych nie powoduje po cichu degradacji do „odmowy”.

## Dodawanie nowej trasy zdolnej do uruchamiania procesów

1. Dodaj prefiks ścieżki do `LOCAL_ONLY_API_PREFIXES` w
   `src/server/authz/routeGuard.ts`
2. Dodaj test w `tests/unit/authz/routeGuard.test.ts`, który sprawdza, czy
   `isLocalOnlyPath()` zwraca true dla nowego prefiksu
3. **Nigdy nie pomijaj tego kroku** — zobacz Twardą regułę nr 15 w `CLAUDE.md`
4. Zdecyduj: czy ta trasa powinna TAKŻE znaleźć się w `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES`?
   Domyślna odpowiedź brzmi **nie**. Włącz tę opcję tylko wtedy, gdy trasę można
   bezpiecznie udostępnić posiadaczowi zakresu manage (tj. NIE uruchamia ona dowolnego
   kodu kontrolowanego przez użytkownika).

## Dodawanie ścieżki możliwej do pominięcia za pomocą zakresu manage

1. Potwierdź, że trasa nie wykonuje kodu ani poleceń dostarczonych przez użytkownika. Jeśli
   je wykonuje, przerwij — ten wyjątek nie jest właściwym narzędziem.
2. Dodaj prefiks do `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` w
   `src/server/authz/routeGuard.ts`
3. Dodaj pokrycie w `tests/unit/authz/management-policy.test.ts` dla wszystkich czterech
   wariantów żądania: bez Bearer (403), Bearer z zakresem manage (zezwól), Bearer bez zakresu manage
   (403) oraz regresji dla każdego prefiksu, która zapewnia, że `/api/cli-tools/runtime/*` pozostaje
   ściśle ograniczone do loopback nawet z Bearer z zakresem manage.
...
| `tests/unit/authz/routeGuard.test.ts`        | Testy jednostkowe funkcji pomocniczych poziomów |
| `tests/unit/authz/management-policy.test.ts` | Testy jednostkowe evaluate()                    |

## Dokumentowanie poziomów bezpieczeństwa w OpenAPI

Podczas dodawania nowej trasy do `docs/openapi.yaml` zastosuj odpowiednie
rozszerzenie dostawcy, jeśli trasa jest sklasyfikowana przez `routeGuard.ts`:

| Klasyfikacja routeGuard.ts                      | Adnotacja YAML                | Egzekwowanie                                              |
| ----------------------------------------------- | ----------------------------- | --------------------------------------------------------- |
| `LOCAL_ONLY_API_PREFIXES`                       | `x-loopback-only: true`       | Bezwarunkowo blokowane spoza loopback                     |
| `ALWAYS_PROTECTED_API_PATHS`                    | `x-always-protected: true`    | Uwierzytelnianie wymagane nawet przy `requireLogin=false` |
| Wewnętrzna trasa admin/debug                    | `x-internal: true`            | Domyślnie ukryta w /dashboard/api-endpoints               |
| Brak (publiczna / standardowe uwierzytelnianie) | (adnotacja nie jest wymagana) | Standardowy dostęp kontrolowany przez `requireLogin`      |

### Walidacja

Dwa skrypty wymuszają spójność między adnotacjami YAML a `routeGuard.ts`:

- `scripts/check/check-openapi-coverage.mjs` — kończy się niepowodzeniem, jeśli pokrycie < 99%
- `scripts/check/check-openapi-security-tiers.mjs` — kończy się niepowodzeniem, jeśli adnotacje `x-loopback-only` lub
  `x-always-protected` odbiegają od stałych określonych na etapie kompilacji

Oba skrypty są uruchamiane w hooku pre-commit oraz w CI.

### Reguła dotycząca wyników fałszywie dodatnich

Jeśli trasa, której NIE ma w stałej w `routeGuard.ts`, ma adnotację `x-always-protected` lub
`x-loopback-only`, skrypt pokrycia kończy się niepowodzeniem. Rozwiązaniem jest zawsze dostosowanie
YAML do tego, co faktycznie egzekwuje `routeGuard.ts` — a nie dodawanie tras do `routeGuard.ts`
bez jednoczesnego zaimplementowania logiki egzekwowania.

---

## Zobacz także

- `docs/security/CLI_TOKEN.md` — token identyfikatora maszyny CLI
- `docs/architecture/AUTHZ_GUIDE.md` — pełny potok autoryzacji
- `docs/frameworks/MCP-SERVER.md` — transporty i zakresy serwera MCP
