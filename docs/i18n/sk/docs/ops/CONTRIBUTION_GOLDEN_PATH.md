...
npm run test:combo:matrix        # zmeny stratégie alebo odosielania
npm run check:known-symbols      # zmeny registrácie stratégie
npm run lint
```
...
- `src/i18n/messages/en.json` je zdrojom používateľského rozhrania; `config/i18n.json` je zdrojom
  lokalizácií.
...
npm run check:cli-i18n          # keď sa zmenia reťazce/katalógy CLI
npm run lint
```
...
npm run build:cli             # zmeny generovaného/zbaleného CLI
npm run check:pack-policy     # zmeny verejného rozhrania balíka
npm run lint
```
