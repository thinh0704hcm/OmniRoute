...
`/api/services/` sú zámerne vylúčené, pretože môžu spúšťať ľubovoľné
...
managementPolicy.evaluate(ctx)
  1. isLocalOnlyPath(path)?
     → loopback                                  → pokračovať ďalej
     → non-loopback, Bearer s rozsahom manage
        AND isLocalOnlyBypassableByManageScope   → povoliť (management_key)
     → inak                                      → zamietnuť 403 LOCAL_ONLY
  2. isInternalModelSyncRequest(ctx)?
     → povoliť (system)
  3. hasValidCliToken(headers)?
     → povoliť (cli) [loopback + kontrola HMAC pomocou timingSafeEqual]
  4. isAlwaysProtectedPath(path) or requireLogin=true?
     → isDashboardSessionAuthenticated?
        → povoliť (dashboard_session)
     → Bearer s rozsahom manage na ceste, ktorú nemožno obísť?
        → povoliť (management_key)
     → zamietnuť 401/403
  5. requireLogin=false?
     → povoliť (anonymous)
```

Vetva s rozsahom manage v kroku 1 je jedinou autentifikovanou cestou, ktorá môže vyhovieť trase LOCAL_ONLY; režim zlyhania autentifikačného backendu vráti 503 (nie 403), aby exspirovaná databáza potichu neprešla na „zamietnuť“.

## Pridanie novej trasy schopnej spúšťať procesy

1. Pridajte prefix cesty do `LOCAL_ONLY_API_PREFIXES` v
   `src/server/authz/routeGuard.ts`
2. Pridajte test do `tests/unit/authz/routeGuard.test.ts`, ktorý overí, že
   `isLocalOnlyPath()` vráti pre nový prefix hodnotu true
3. **Tento krok nikdy nevynechávajte** — pozrite si pevné pravidlo č. 15 v `CLAUDE.md`
4. Rozhodnite: patrí táto trasa AJ do `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES`?
   Predvolená odpoveď je **nie**. Povoľte to iba vtedy, keď je bezpečné sprístupniť
   trasu držiteľovi rozsahu manage (t. j. NESMIE spúšťať ľubovoľný kód riadený používateľom).

## Pridanie cesty, ktorú možno obísť pomocou rozsahu manage

1. Overte, že trasa nespúšťa kód ani príkazy dodané používateľom. Ak ich
   spúšťa, zastavte sa — táto výnimka nie je vhodným nástrojom.
2. Pridajte prefix do `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` v
   `src/server/authz/routeGuard.ts`
3. Pridajte pokrytie v `tests/unit/authz/management-policy.test.ts` pre všetky štyri
   podoby požiadavky: bez Bearer (403), Bearer s rozsahom manage (povoliť), Bearer
   bez rozsahu manage (403) a regresný test pre každý prefix, ktorý overí, že
   `/api/cli-tools/runtime/*` zostáva striktne obmedzený na loopback aj s Bearer
   s rozsahom manage.
...
| Súbor                                        | Účel                                      |
| -------------------------------------------- | ----------------------------------------- |
| `src/server/authz/routeGuard.ts`             | Konštanty a pomocné funkcie               |
| `src/server/authz/policies/management.ts`    | Logika vyhodnocovania                     |
| `tests/unit/authz/routeGuard.test.ts`        | Jednotkové testy pomocných funkcií úrovní |
| `tests/unit/authz/management-policy.test.ts` | Jednotkové testy pre evaluate()           |

## Dokumentovanie bezpečnostných úrovní v OpenAPI

Pri pridávaní novej trasy do `docs/openapi.yaml` použite zodpovedajúce
rozšírenie dodávateľa, ak je trasa klasifikovaná v `routeGuard.ts`:

| Klasifikácia v routeGuard.ts               | Anotácia YAML              | Vynucovanie                                            |
| ------------------------------------------ | -------------------------- | ------------------------------------------------------ |
| `LOCAL_ONLY_API_PREFIXES`                  | `x-loopback-only: true`    | Bezpodmienečne blokované pre požiadavky mimo loopbacku |
| `ALWAYS_PROTECTED_API_PATHS`               | `x-always-protected: true` | Autentifikácia sa vyžaduje aj pri `requireLogin=false` |
| Interná admin/debug trasa                  | `x-internal: true`         | Predvolene skrytá v /dashboard/api-endpoints           |
| Žiadna (verejná/štandardná autentifikácia) | (anotácia nie je potrebná) | Štandardný prístup riadený pomocou `requireLogin`      |

### Overovanie

Konzistenciu medzi anotáciami YAML a `routeGuard.ts` vynucujú dva skripty:

- `scripts/check/check-openapi-coverage.mjs` — zlyhá, ak je pokrytie < 99 %
- `scripts/check/check-openapi-security-tiers.mjs` — zlyhá, ak sa anotácie
  `x-loopback-only` alebo `x-always-protected` líšia od konštánt pri kompilácii

Oba skripty sa spúšťajú v pre-commit hooku aj v CI.

### Pravidlo pre falošne pozitívne výsledky

Ak je trasa anotovaná pomocou `x-always-protected` alebo `x-loopback-only`, ale NIE JE
uvedená v konštante v `routeGuard.ts`, skript pokrytia zlyhá. Riešením je vždy zosúladiť
YAML s tým, čo `routeGuard.ts` skutočne vynucuje — nie pridávať trasy do `routeGuard.ts`
bez súčasnej implementácie logiky vynucovania.

---

## Pozri tiež

- `docs/security/CLI_TOKEN.md` — token ID zariadenia pre CLI
- `docs/architecture/AUTHZ_GUIDE.md` — kompletný autorizačný proces
- `docs/frameworks/MCP-SERVER.md` — transporty a rozsahy servera MCP
