...
**Uygulayan:** `isAlwaysProtectedPath(path)` → `requireLogin=false` atlamasını geç
...
managementPolicy.evaluate(ctx)
  1. isLocalOnlyPath(path)?
     → loopback                                  → sonraki adıma geç
     → loopback olmayan, manage kapsamlı Bearer
        VE isLocalOnlyBypassableByManageScope    → izin ver (management_key)
     → aksi takdirde                              → reddet 403 LOCAL_ONLY
  2. isInternalModelSyncRequest(ctx)?
     → izin ver (system)
  3. hasValidCliToken(headers)?
     → izin ver (cli) [loopback + timingSafeEqual HMAC denetimi]
  4. isAlwaysProtectedPath(path) veya requireLogin=true?
     → isDashboardSessionAuthenticated?
        → izin ver (dashboard_session)
     → atlanamaz bir yolda manage kapsamlı Bearer?
        → izin ver (management_key)
     → reddet 401/403
  5. requireLogin=false?
     → izin ver (anonymous)
```

1. adımdaki manage kapsamı dalı, bir LOCAL_ONLY rotasını karşılayabilen tek kimliği doğrulanmış yoldur; kimlik doğrulama arka ucu hata modu 503 (403 değil) döndürür, böylece süresi dolmuş bir DB sessizce "reddet" durumuna indirgenmez.

## Yeni bir süreç başlatabilen rota ekleme

1. Yol ön ekini `src/server/authz/routeGuard.ts` içindeki
   `LOCAL_ONLY_API_PREFIXES` sabitine ekleyin
2. `tests/unit/authz/routeGuard.test.ts` içine, yeni ön ek için
   `isLocalOnlyPath()` işlevinin true döndürdüğünü doğrulayan bir test ekleyin
3. **Bu adımı asla atlamayın** — `CLAUDE.md` içindeki Kesin Kural #15'e bakın
4. Karar verin: Bu rota AYRICA `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` içinde yer almalı mı?
   Varsayılan yanıt **hayır**dır. Yalnızca rota bir manage kapsamı sahibine açılmak için
   güvenliyse (yani kullanıcı tarafından denetlenen herhangi bir kodu çalıştırmıyorsa)
   bunu açıkça etkinleştirin.

## Manage kapsamıyla atlanabilir bir yol ekleme

1. Rotanın kullanıcı tarafından sağlanan kodu veya komutları çalıştırmadığını doğrulayın. Çalıştırıyorsa
   durun — bu istisna bunun için uygun araç değildir.
2. Ön eki `src/server/authz/routeGuard.ts` içindeki
   `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` sabitine ekleyin
3. `tests/unit/authz/management-policy.test.ts` içine şu dört
   istek biçiminin tümünü kapsayan testler ekleyin: Bearer yok (403), manage Bearer (izin ver),
   manage olmayan Bearer (403) ve `/api/cli-tools/runtime/*` yolunun bir manage Bearer
   olsa bile katı biçimde loopback olarak kaldığını doğrulayan ön ek bazlı regresyon testi.

## Dosyalar

| Dosya                                        | Amaç                                    |
| -------------------------------------------- | --------------------------------------- |
| `src/server/authz/routeGuard.ts`             | Sabitler ve yardımcı işlevler           |
| `src/server/authz/policies/management.ts`    | Değerlendirme mantığı                   |
| `tests/unit/authz/routeGuard.test.ts`        | Katman yardımcıları için birim testleri |
| `tests/unit/authz/management-policy.test.ts` | evaluate() için birim testleri          |

## OpenAPI'de Güvenlik Katmanlarını Belgeleme

`docs/openapi.yaml` dosyasına yeni bir rota eklerken, rota `routeGuard.ts`
tarafından sınıflandırılmışsa karşılık gelen sağlayıcı uzantısını uygulayın:

| routeGuard.ts sınıflandırması                  | YAML ek açıklaması         | Uygulama                                                      |
| ---------------------------------------------- | -------------------------- | ------------------------------------------------------------- |
| `LOCAL_ONLY_API_PREFIXES`                      | `x-loopback-only: true`    | Loopback olmayan erişimler koşulsuz engellenir                |
| `ALWAYS_PROTECTED_API_PATHS`                   | `x-always-protected: true` | `requireLogin=false` olsa bile kimlik doğrulama gerekir       |
| Dahili yönetici/hata ayıklama rotası           | `x-internal: true`         | Varsayılan olarak /dashboard/api-endpoints üzerinden gizlenir |
| Yok (herkese açık / standart kimlik doğrulama) | (ek açıklama gerekmez)     | Standart `requireLogin` denetimli erişim                      |

### Doğrulama

İki betik, YAML ek açıklamaları ile `routeGuard.ts` arasındaki tutarlılığı zorunlu kılar:

- `scripts/check/check-openapi-coverage.mjs` — kapsam <%99 ise başarısız olur
- `scripts/check/check-openapi-security-tiers.mjs` — `x-loopback-only` veya
  `x-always-protected` ek açıklamaları derleme zamanı sabitlerinden farklıysa başarısız olur

Her iki betik de ön gönderim kancasında ve CI'da çalışır.

### Yanlış Pozitif Kuralı

`routeGuard.ts` sabitinde OLMAYAN bir rotaya `x-always-protected` veya `x-loopback-only`
ek açıklaması eklenirse kapsam betiği başarısız olur. Çözüm her zaman YAML'ı
`routeGuard.ts` tarafından fiilen uygulanan kurallarla uyumlu hâle getirmektir; uygulama
mantığını da eklemeden `routeGuard.ts` dosyasına rota eklemek değildir.

---

## Ayrıca bkz.

- `docs/security/CLI_TOKEN.md` — CLI makine kimliği belirteci
- `docs/architecture/AUTHZ_GUIDE.md` — eksiksiz yetkilendirme işlem hattı
- `docs/frameworks/MCP-SERVER.md` — MCP sunucu aktarımları ve kapsamları
