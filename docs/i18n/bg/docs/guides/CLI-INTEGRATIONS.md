...
OmniRoute предлага набор от команди `setup-*`, които конфигурират CLI за кодиране (Codex, Claude Code, OpenCode, Cline, …) да използва OmniRoute като свой бекенд — така инструментът комуникира с **една** крайна точка, а OmniRoute пренасочва към правилния доставчик с автоматично превключване при отказ. Всяка команда чете **актуалния** каталог с модели от работещ OmniRoute (локален или отдалечен) и записва собствения конфигурационен файл на инструмента на **вашата** машина. API ключът се реферира от променлива на средата, където инструментът поддържа това. Командите, които запазват локален за инструмента файл със среда, са отбелязани по-долу.

Има и общ стартер — `omniroute run <target>` — който стартира `claude`, `codex`, `aider`, `goose`, `opencode`, `qwen` или `gemini` с инжектирана правилната среда, без да записва никаква конфигурация. Целите и техните псевдоними идват от каноничния манифест `bin/cli/cli-manifest.mjs` (`claude-code|cc|anthropic`, `codex-cli|openai-codex|openai`, `goose-cli`, `open-code`, `qwen-code`, `gemini-cli`), а `omniroute completion` предлага същите целеви думи, извлечени от манифеста. Налични остават и наследените стартери за всеки инструмент — `omniroute launch` (Claude Code) и `omniroute launch-codex` (Codex).

Включването на доставчици е достъпно от същия локален/отдалечен контекст. Посочените по-долу команди, ориентирани към API, поддържат удостоверяването за управление отделно от идентификационните данни на доставчика и никога не отпечатват идентификационни данни в структуриран изход:
...
За скриптове предпочитайте `--credential-stdin` или `--credential-env`; `--credential` е запазен за контролирана локална употреба. `providers remove` изисква `--yes` на неинтерактивен терминал, а всичките пет команди спазват активния контекст или глобалните опции `--base-url`/`--api-key`.

Селекторите на доставчици отхвърлят двусмислени ID префикси, имена или имена на доставчици; използвайте пълно ID на връзката, когато няколко връзки съвпадат. Командите за създаване и редактиране прочитат обратно запазената връзка, а премахването проверява дали тя вече не е четима. Импортът пропуска съществуваща двойка доставчик/име. Импортираните записи не могат да презаписват крайната точка за управление, контекста или идентификационните данни за управление, предоставени на CLI.

За еднократната, ръчно написана базова настройка на двете най-богати интеграции, вижте подробните ръководства за всеки инструмент:
...
- [VS Code Copilot Chat](./VSCODE-COPILOT.md) — разширението OmniCopilot; то също може да изпълнява тези команди `setup-*` вместо вас от редактора
