...
MCP инструментите се удостоверяват чрез обхвати на API ключове. Прилагането на обхватите е централизирано в
`open-sse/mcp-server/scopeEnforcement.ts`. Всеки инструмент изисква конкретни обхвати:
...
Достъпът до HTTP/SSE MCP транспорта (`/api/mcp/*`) извън loopback изисква изключението LOCAL_ONLY за `/api/mcp/` (вижте `docs/security/ROUTE_GUARD_TIERS.md`). Исторически това изключение приемаше само API ключ с пълен обхват `manage`/`admin` — твърде широк за клиент, който трябва единствено да комуникира с MCP. `src/shared/constants/managementScopes.ts` вече експортира `MCP_CONNECT_SCOPE = "mcp:connect"`: допълващ, тясно ограничен обхват (по същия прецедент като `SELF_USAGE_SCOPE`), който разрешава САМО заобикалянето за `/api/mcp/` в `src/server/authz/policies/management.ts` — той не предоставя достъп до други маршрути за управление и умишлено НЕ е включен в `MANAGEMENT_API_KEY_SCOPES`. Ключ с `manage`/`admin` продължава да преминава през изключението без промяна; `mcp:connect` е алтернатива с по-ниски привилегии за отдалечени клиенти, използващи само MCP, и се проверява чрез `hasMcpConnectOrManageScope()`.

### Обвързване на HTTP обхвати за всеки ключ (#7895)

При HTTP/SSE `open-sse/mcp-server/httpTransport.ts` вече извлича действителните `api_keys.scopes` на клиента чрез `resolveMcpCallerAuthInfo()` (`open-sse/mcp-server/httpAuthContext.ts`) и ги предава към `transport.handleRequest(req, { authInfo })` на MCP SDK, така че `extra.authInfo.scopes`, достигащ до всяко извикване на инструмент, да отразява собствените обхвати на Bearer ключа. `resolveCallerScopeContext()` от `scopeEnforcement.ts` вече даваше приоритет на `authInfo` пред `_meta` и резервната стойност от променливата на средата `OMNIROUTE_MCP_SCOPES` — тази промяна само попълва този първи източник с най-висок приоритет, който преди не получаваше данни през HTTP. Когато не бъде открит API ключ (няма заглавка или ключът е невалиден), `authInfo` остава `undefined` и извличането продължава без промяна по съществуващата верига `meta`/променлива на средата. Това НЕ променя стойността по подразбиране на `OMNIROUTE_MCP_ENFORCE_SCOPES` — прилагането на ограниченията все още трябва да бъде изрично активирано; тази промяна само гарантира, че пътят за отделния ключ има предимство, след като прилагането бъде активирано. stdio няма идентичност за отделен клиент (вижте `mcpCallerIdentity.ts`) и не се засяга — той продължава да използва резервната верига `_meta`/променлива на средата.
