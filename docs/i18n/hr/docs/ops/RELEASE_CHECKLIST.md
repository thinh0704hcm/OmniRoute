...
> Pojednostavljen tijek izdanja koji koristi vještine Claude Codea za automatizaciju.
>
> **Održavajte red čekanja/granu zelenima između izdanja:** pogledajte [RELEASE_GREEN.md](./RELEASE_GREEN.md)
> (obitelj `/green-prs` + `npm run check:release-green` + `/babysit` + noćno izvršavanje). Povremeno
> pokretanje ovoga — a osobito **prije** ovog kontrolnog popisa — osigurava da PR izdanja u početku bude zelen.
