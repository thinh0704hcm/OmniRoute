...
> Απλοποιημένη ροή έκδοσης που αξιοποιεί τις δεξιότητες του Claude Code για αυτοματοποίηση.
>
> **Διατηρείτε την ουρά/τον κλάδο σε πράσινη κατάσταση μεταξύ των εκδόσεων:** δείτε το [RELEASE_GREEN.md](./RELEASE_GREEN.md)
> (οικογένεια `/green-prs` + `npm run check:release-green` + `/babysit` + νυχτερινή εκτέλεση). Η περιοδική εκτέλεση
> αυτής της διαδικασίας — και ειδικά **πριν** από αυτήν τη λίστα ελέγχου — εξασφαλίζει ότι το PR της έκδοσης ξεκινά σε πράσινη κατάσταση.
