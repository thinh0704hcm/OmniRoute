...
> **Функция:** Playground Studio — единое рабочее пространство для тестирования ИИ по адресу `/dashboard/playground`.
> **Планы:** `17-playground-studio-redesign.plan.md` + `_orchestration/master-plan-group-C.md`
...
- `ToolsBuilder.tsx` — добавление, редактирование и удаление `tools[]` с редактором схемы JSON для каждого инструмента.
  Проверяет параметры с помощью `ToolDefinitionSchema` (Zod).
- `StructuredOutputEditor.tsx` — переключатель режима JSON и редактор схемы JSON.
  Проверяет ответ на соответствие схеме с помощью `StructuredOutputSchema` (Zod).
...
`ExportCodeModal.tsx` использует `codeExport.ts` для генерации фрагментов curl / Python / TypeScript
