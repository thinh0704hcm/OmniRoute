...
> **Кратко**: `open-sse/` — это основное потоковое ядро, обслуживающее каждый запрос к LLM в OmniRoute. Оно содержит около 1 400 файлов, реализующих конвейер обработки запросов, исполнители, сервисы, MCP-сервер и слой преобразования форматов. В этом руководстве объясняется, как все эти компоненты связаны между собой.

**Исходный код:** `open-sse/` (пакет рабочего пространства, около 1 440 `.ts`-файлов без учёта тестов)
...
### Этап 1: Маршрутизация (services/combo.ts)
...
❌ **Синхронные вызовы БД в `combo.ts`** — выполняйте предварительные вычисления и кешируйте
...
- [ARCHITECTURE.md](../architecture/ARCHITECTURE.md) — высокоуровневая архитектура
- [CODEBASE_DOCUMENTATION.md](../architecture/CODEBASE_DOCUMENTATION.md) — инженерный справочник
- [REPOSITORY_MAP.md](../architecture/REPOSITORY_MAP.md) — описание каждого каталога
- [AUTO-COMBO.md](../routing/AUTO-COMBO.md) — оценка по 16 факторам
- [MCP-SERVER.md](./MCP-SERVER.md) — сервер MCP
- [A2A-SERVER.md](./A2A-SERVER.md) — сервер A2A
- Исходный код: `open-sse/` (более 400 файлов, ~143 тыс. строк кода)
