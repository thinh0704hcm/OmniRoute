...
សាខា manage-scope ក្នុងជំហានទី 1 គឺជាផ្លូវដែលបានផ្ទៀងផ្ទាត់អត្តសញ្ញាណតែមួយគត់ ដែលអាចបំពេញលក្ខខណ្ឌសម្រាប់ route ប្រភេទ
LOCAL_ONLY បាន។ ក្នុងករណី auth-backend បរាជ័យ វានឹងត្រឡប់ 503 (មិនមែន 403 ទេ) ដូច្នេះ
DB ដែលផុតកំណត់នឹងមិនបន្ទាបស្ថានភាពដោយស្ងៀមស្ងាត់ទៅជា "deny" ឡើយ។

## ការបន្ថែម route ថ្មីដែលអាច spawn បាន

1. បន្ថែមបុព្វបទ path ទៅ `LOCAL_ONLY_API_PREFIXES` ក្នុង
   `src/server/authz/routeGuard.ts`
2. បន្ថែមតេស្តមួយក្នុង `tests/unit/authz/routeGuard.test.ts` ដើម្បីអះអាងថា
   `isLocalOnlyPath()` ត្រឡប់ true សម្រាប់បុព្វបទថ្មី
3. **កុំរំលងជំហាននេះជាដាច់ខាត** — សូមមើល Hard Rule #15 ក្នុង `CLAUDE.md`
4. សម្រេចថា៖ តើ route នេះក៏គួរស្ថិតក្នុង `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` ដែរឬទេ?
   ចម្លើយលំនាំដើមគឺ **ទេ**។ ជ្រើសចូលតែក្នុងករណីដែល route នោះមានសុវត្ថិភាពសម្រាប់បើកឱ្យ
   អ្នកកាន់ manage-scope ចូលប្រើប៉ុណ្ណោះ (មានន័យថា វាមិន spawn កូដដែលគ្រប់គ្រងដោយអ្នកប្រើតាមអំពើចិត្តឡើយ)។

## ការបន្ថែម path ដែលអាចរំលងតាម manage-scope បាន

1. បញ្ជាក់ថា route មិនប្រតិបត្តិកូដ ឬ command ដែលផ្តល់ដោយអ្នកប្រើទេ។ ប្រសិនបើវា
   ធ្វើដូច្នោះ សូមឈប់ — ករណីលើកលែងនេះមិនមែនជាឧបករណ៍ត្រឹមត្រូវឡើយ។
2. បន្ថែមបុព្វបទទៅចុងបញ្ជី `LOCAL_ONLY_MANAGE_SCOPE_BYPASS_PREFIXES` ក្នុង
   `src/server/authz/routeGuard.ts`
3. បន្ថែមការគ្របដណ្តប់ក្នុង `tests/unit/authz/management-policy.test.ts` សម្រាប់ទម្រង់
   request ទាំងបួន៖ គ្មាន Bearer (403), manage Bearer (អនុញ្ញាត), non-manage Bearer
   (403) និង regression តាមបុព្វបទ ដើម្បីធានាថា `/api/cli-tools/runtime/*` នៅតែ
   ទាមទារ loopback យ៉ាងតឹងរ៉ឹង ទោះបីមាន manage Bearer ក៏ដោយ។

## ឯកសារ

| ឯកសារ                                        | គោលបំណង                              |
| -------------------------------------------- | ------------------------------------ |
| `src/server/authz/routeGuard.ts`             | ថេរ និងមុខងារជំនួយ                   |
| `src/server/authz/policies/management.ts`    | តក្កវិជ្ជានៃការវាយតម្លៃ              |
| `tests/unit/authz/routeGuard.test.ts`        | តេស្តឯកតាសម្រាប់មុខងារជំនួយតាមកម្រិត |
| `tests/unit/authz/management-policy.test.ts` | តេស្តឯកតាសម្រាប់ evaluate()          |

## ការចងក្រងឯកសារអំពីកម្រិតសុវត្ថិភាពក្នុង OpenAPI

នៅពេលបន្ថែម route ថ្មីទៅ `docs/openapi.yaml` សូមអនុវត្ត vendor extension ដែលត្រូវគ្នា
ប្រសិនបើ route នោះត្រូវបានចាត់ថ្នាក់ដោយ `routeGuard.ts`៖

| ការចាត់ថ្នាក់ក្នុង routeGuard.ts | YAML annotation            | ការអនុវត្តការគ្រប់គ្រង                           |
| -------------------------------- | -------------------------- | ------------------------------------------------ |
| `LOCAL_ONLY_API_PREFIXES`        | `x-loopback-only: true`    | ទប់ស្កាត់ពី non-loopback ដោយគ្មានលក្ខខណ្ឌ        |
| `ALWAYS_PROTECTED_API_PATHS`     | `x-always-protected: true` | ទាមទារ auth ទោះបីជា `requireLogin=false` ក៏ដោយ   |
| route admin/debug ផ្ទៃក្នុង      | `x-internal: true`         | លាក់ពី /dashboard/api-endpoints តាមលំនាំដើម      |
| គ្មាន (សាធារណៈ / auth ស្តង់ដារ)  | (មិនត្រូវការ annotation)   | ការចូលប្រើស្តង់ដារដែលគ្រប់គ្រងដោយ `requireLogin` |

### ការផ្ទៀងផ្ទាត់

script ចំនួនពីរអនុវត្តភាពស៊ីសង្វាក់គ្នារវាង YAML annotation និង `routeGuard.ts`៖

- `scripts/check/check-openapi-coverage.mjs` — បរាជ័យប្រសិនបើការគ្របដណ្តប់ < 99%
- `scripts/check/check-openapi-security-tiers.mjs` — បរាជ័យប្រសិនបើ annotation `x-loopback-only` ឬ
  `x-always-protected` ខុសពីថេរនៅពេល compile

script ទាំងពីរដំណើរការក្នុង pre-commit hook និងក្នុង CI។

### ច្បាប់សម្រាប់ False Positive

ប្រសិនបើ `x-always-protected` ឬ `x-loopback-only` ត្រូវបានដាក់ annotation លើ route ដែលមិនស្ថិតក្នុង
ថេររបស់ `routeGuard.ts` នោះ coverage script នឹងបរាជ័យ។ ដំណោះស្រាយគឺត្រូវធ្វើឱ្យ
YAML ស្របតាមអ្វីដែល `routeGuard.ts` អនុវត្តជាក់ស្តែងជានិច្ច — មិនមែនបន្ថែម route ទៅ `routeGuard.ts`
ដោយមិនអនុវត្តតក្កវិជ្ជានៃការគ្រប់គ្រងនោះផងទេ។

---

## សូមមើលផងដែរ

- `docs/security/CLI_TOKEN.md` — ថូខឹន machine-ID របស់ CLI
- `docs/architecture/AUTHZ_GUIDE.md` — ដំណើរការផ្តល់សិទ្ធិពេញលេញ
- `docs/frameworks/MCP-SERVER.md` — មធ្យោបាយដឹកជញ្ជូន និងវិសាលភាពរបស់ម៉ាស៊ីនបម្រើ MCP
