...
- obsolètes
- incomplets
- dépourvus de cookies envoyés uniquement lors des requêtes authentifiées
...
→ Réseau
→ Actualiser la page
→ Ouvrir une requête de chat ou de conversation
→ En-têtes de la requête
→ Cookie
```

L’en-tête de requête `Cookie` contient les informations d’authentification exactes que votre navigateur a utilisées avec succès.

Pour la plupart des fournisseurs Web Cookie, il s’agit de la valeur à coller dans OmniRoute.

---

# Configuration générale

Le processus de configuration est identique pour la plupart des fournisseurs Web Cookie.

1. Connectez-vous au site web du fournisseur.
...
9. Sélectionnez votre fournisseur Web Cookie.
10. Collez les identifiants.
11. Cliquez sur **Tester la connexion**.
12. Enregistrez le fournisseur.

Les identifiants exacts requis dépendent du fournisseur.

---

# Formats des identifiants des fournisseurs
...
Vérifiez que les identifiants ont été copiés depuis :
...
## Le test de connexion réussit, mais les requêtes échouent
...
- Copiez les identifiants depuis une nouvelle requête authentifiée.
