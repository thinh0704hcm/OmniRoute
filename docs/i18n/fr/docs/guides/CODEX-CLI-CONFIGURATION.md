...
Si vous avez installé Codex via l’application de bureau ChatGPT, le binaire `codex` peut
...
Codex vérifie que la variable d’environnement désignée par `env_key` existe
...
### Champs de configuration des jetons
...
Guide complet : [THINKING_BUDGET.md](./THINKING_BUDGET.md).
...
Les profils permettent de changer de modèle et de fenêtre de contexte avec un seul indicateur. Chaque profil est un fichier plat
`~/.codex/<name>.config.toml` qui se superpose au fichier `config.toml` de base.

> **Règle de nommage (Codex CLI v0.137+) :** le fichier doit être `~/.codex/<name>.config.toml` — **sans préfixe `profile-`**.
> La CLI résout `-p kimi-k27` en `~/.codex/kimi-k27.config.toml`. Si le fichier est introuvable, la configuration par défaut s'applique silencieusement.
...
Codex est également une cible des deux points d’entrée génériques pilotés par manifeste
...
### Nouveaux champs de `[model_providers.<id>]`
...
### Authentification Amazon Bedrock (v0.140)
...
## Claude Code — configuration équivalente

| Codex CLI (`config.toml`)         | Claude Code (variable d’environnement) | Effet                                |
| --------------------------------- | -------------------------------------- | ------------------------------------ |
| `tool_output_token_limit = 32768` | _(non directement exposé)_             | Limite d’historique par outil        |
| `model_context_window = 400000`   | _(déterminée par le modèle)_           | Fenêtre de contexte                  |
...
## Référence rapide — options de la CLI
...
Discussions connexes : [#7126](https://github.com/diegosouzapw/OmniRoute/discussions/7126) (interruptions des tâches longues), [#5718](https://github.com/diegosouzapw/OmniRoute/discussions/5718) (pourquoi l’affinité est désactivée par défaut). Suivi : [#7287](https://github.com/diegosouzapw/OmniRoute/issues/7287).
...
**Valeur par défaut :** `STREAM_IDLE_TIMEOUT_MS = 600000` (10 minutes). Lorsqu’il n’est pas défini, il hérite de `REQUEST_TIMEOUT_MS` ; la valeur de référence partagée est également 600000. Consultez [`docs/guides/SETUP_GUIDE.md`](SETUP_GUIDE.md) → Délais d’expiration.
...
3. Conservez le fichier `config.toml` Codex habituel (`wire_api = "responses"`, `base_url` correct, `OMNIROUTE_API_KEY`) — aucun réglage d’affinité ou d’inactivité côté Codex n’existe pour ces deux comportements.
4. Redémarrez OmniRoute, puis lancez la tâche Codex de longue durée.
...
Consultez [Tâches de longue durée](#long-running-tasks). Activez l’affinité de session (avec un TTL supérieur à la durée de la tâche) et augmentez ou désactivez `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Recherchez `Idle timeout: no data from` dans les journaux d’OmniRoute avec grep.
